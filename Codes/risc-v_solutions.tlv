...
\TLV
   ...
   |cpu
      @0
         $reset = *reset;                                                            //Reset for different modules
         
         $pc[31:0] = >>1$reset ? 32'd0 :                                             //Program counter update supporting load,jump and branch instructions
                     >>3$valid_load ? >>3$inc_pc:
                     >>3$taken_br? >>3$br_tgt_pc:
                     (>>3$valid_jump && >>3$is_jal) ? >>3$br_tgt_pc :
                     (>>3$valid_jump && >>3$is_jalr) ? >>3$jalr_tgt_pc : >>1$inc_pc;

         $imem_rd_en = !$reset;                                                      //Enable instruction memory read for every cycle 
         $imem_rd_addr[M4_IMEM_INDEX_CNT-1:0] = $pc[M4_IMEM_INDEX_CNT+1:2];          //PC -> incremented by 4, last two bits always zero 
         
      @1
         $inc_pc[31:0] = $pc + 32'd4;                                                //increment PC for next instruction and store in intermediate signal $inc_pc 
         
         $instr[31:0] = $imem_rd_data;                                               //Read instruction from instruction memory address pointed by PC
         
         $is_i_instr = $instr[6:2] ==? 5'b0000x ||                                   // Various instruction type decode 
                       $instr[6:2] ==? 5'b001x0 ||
                       $instr[6:2] ==? 5'b11001 ;
         ...
         $imm[31:0] = $is_i_instr ? { {21{$instr[31]}},$instr[30:20]} :                                         //Immediate value based on the bit patterns for different instructions
                      $is_s_instr ? { {21{$instr[31]}},$instr[30:25],$instr[11:7]} :
                      $is_b_instr ? { {20{$instr[31]}},$instr[7],$instr[30:25],$instr[11:8],1'b0} :
                      $is_j_instr ? { $instr[31:12],12'b0 } :
                      $is_u_instr ? { {12{$instr[31]}},$instr[19:12],$instr[20],$instr[30:21],1'b0} : 32'b0;
         
         $funct3_valid = $is_i_instr || $is_r_instr || $is_s_instr || $is_b_instr ;                             //Different instruction field validity
